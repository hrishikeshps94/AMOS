...
from monai.transforms import LoadImaged,AddChanneld,Orientationd,Spacingd,ScaleIntensityRanged,CropForegroundd,\
    RandCropByPosNegLabeld,RandFlipd,RandRotate90d,RandShiftIntensityd,ToTensord,Compose
...
class CustomDataset(Dataset):
    def __init__(self,args,is_train=True):
        ...
        if is_train:
            self.transforms = Compose([LoadImaged(keys=["image", "label"]),AddChanneld(keys=["image", "label"])]
    ...
    def __getitem__(self, idx):
        out = self.transforms(self.datalist[idx])
        return out
